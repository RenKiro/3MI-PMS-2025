...
class PayrollApp(ctk.CTk):
    ...
    def setup_employee_section(self):
        # Create Employee Management content
        employee_management_frame = tb.Frame(self.content_frame, bootstyle=LIGHT)
        employee_management_frame.pack(fill=BOTH, expand=True)

        # Employee Form
        frame_form = tb.Frame(employee_management_frame)
        frame_form.pack(pady=20)

        tb.Label(frame_form, text="Full Name").grid(row=0, column=0, padx=10, pady=5)
        self.entry_name = tb.Entry(frame_form)
        self.entry_name.grid(row=0, column=1)

        tb.Label(frame_form, text="Position").grid(row=1, column=0, padx=10, pady=5)
        self.entry_position = tb.Entry(frame_form)
        self.entry_position.grid(row=1, column=1)

        tb.Label(frame_form, text="Salary").grid(row=2, column=0, padx=10, pady=5)
        self.entry_salary = tb.Entry(frame_form)
        self.entry_salary.grid(row=2, column=1)

        tb.Label(frame_form, text="Mobile No.").grid(row=3, column=0, padx=10, pady=5)
        self.entry_mobile_no = tb.Entry(frame_form)
        self.entry_mobile_no.grid(row=3, column=1)

        tb.Label(frame_form, text="Email").grid(row=4, column=0, padx=10, pady=5)
        self.entry_email = tb.Entry(frame_form)
        self.entry_email.grid(row=4, column=1)

        tb.Label(frame_form, text="Birthdate").grid(row=5, column=0, padx=10, pady=5)
        self.entry_birthdate = tb.Entry(frame_form)
        self.entry_birthdate.grid(row=5, column=1)

        tb.Label(frame_form, text="Nationality").grid(row=6, column=0, padx=10, pady=5)
        self.entry_nationality = tb.Entry(frame_form)
        self.entry_nationality.grid(row=6, column=1)

        # Buttons for Employee Management
        frame_buttons = tb.Frame(employee_management_frame)
        frame_buttons.pack(pady=10)

        tb.Button(frame_buttons, text="Add", command=self.add_employee).grid(row=0, column=0, padx=5)
        tb.Button(frame_buttons, text="Clear", command=self.clear_fields).grid(row=0, column=1, padx=5)

        # Employee Table with Scrollbars
        frame_table = tb.Frame(employee_management_frame)
        frame_table.pack(fill="both", expand=True)

        # Vertical Scrollbar
        scrollbar_vertical = tb.Scrollbar(frame_table, orient="vertical")
        scrollbar_vertical.pack(side="right", fill="y")

        # Horizontal Scrollbar
        scrollbar_horizontal = tb.Scrollbar(frame_table, orient="horizontal")
        scrollbar_horizontal.pack(side="bottom", fill="x")

        employee_table = tb.Treeview(
            frame_table,
            columns=("ID", "Name", "Position", "Salary", "Phone", "Email", "Birthdate", "Nationality"),
            show="headings",
        ...
        scrollbar_vertical.config(command=employee_table.yview)
        scrollbar_horizontal.config(command=employee_table.xview)

        # Define column headings with center alignment
        employee_table.heading("ID", text="ID")
        employee_table.heading("Name", text="Name")
        employee_table.heading("Position", text="Position")
        ...
        employee_table.heading("Nationality", text="Nationality")

        # Set column alignment to center
        for col in ("ID", "Name", "Position", "Salary", "Phone", "Email", "Birthdate", "Nationality"):
            employee_table.column(col, anchor="center", stretch=True)
